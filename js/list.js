...
editButtons.forEach(button => {
  button.addEventListener('click', function() {
    ...
    const amount = row.querySelector('.amount-cell').textContent.trim().replace('円', '').replace(/,/g, ''); // 金額から「円」とカンマを削除
...
document.querySelectorAll('.delete-button').forEach(button => {
  button.addEventListener('click', function() { // 削除ボタンがクリックされたときの処理
