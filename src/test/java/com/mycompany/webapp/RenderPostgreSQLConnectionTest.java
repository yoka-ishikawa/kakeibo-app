...
public class RenderPostgreSQLConnectionTest {
    ...
    public void testRenderPostgreSQLConnection() {
        ...
        try {
            ...
            System.out
                    .println("✅ データベース接続成功: " + connection.getMetaData().getDatabaseProductName());

            // テーブル一覧確認
            Statement statement = connection.createStatement();
            ResultSet tables = connection.getMetaData().getTables(null, null, "tb_info_kanri",
                    new String[] {"TABLE"});
            ...
            if (tables.next()) {
                ...
                // データ件数確認
                ResultSet rs =
                        statement.executeQuery("SELECT COUNT(*) as count FROM tb_info_kanri");
                if (rs.next()) {
                ...
                while (rs.next()) {
                    System.out.printf("  ID: %d, 種別: %s, 金額: %d円, 内容: %s, 日付: %s%n",
                            rs.getLong("id"), rs.getString("syubetu"), rs.getInt("kingaku"),
                            rs.getString("naisyo"), rs.getDate("hiduke"));
                }
                rs.close();
            } else {
