...
public class RenderMonitoringService {
    ...
    public CompletableFuture<Void> startDeploymentMonitoring() {
        ...
        try {
            ...
            } else if ("building".equals(latestDeployStatus)
                    || "deploying".equals(latestDeployStatus)) {
    ...
    private String checkLatestDeploymentStatus(String apiKey) {
        try {
            ...
            String url =
                    RENDER_API_BASE_URL + "/services/" + RENDER_SERVICE_ID + "/deploys?limit=1";
    ...
    private String getRecentErrorLogs(String apiKey) {
        try {
            ...
            String url = RENDER_API_BASE_URL + "/logs" + "?resource=" + RENDER_SERVICE_ID
                    + "&level=error" + "&limit=5";
    ...
    private void sendDeploymentFailureNotification(String status, String errorLogs) {
        try {
            ...
            // 既存のメソッドを使用
            notificationService.sendDeployFailureNotificationWithLog("kakeibo-app", "latest",
                    errorLogs, timestamp);
    ...
    private void sendMonitoringErrorNotification(String errorMessage) {
        try {
            ...
            // 既存のメソッドを使用
            notificationService.sendConnectionErrorDetails("Monitoring Error", errorMessage,
     ...
     * デプロイ失敗用Flexメッセージ作成
     */
    private String createDeploymentFailureFlexMessage(String status, String errorLogs,
            String timestamp) {
        try {
            // エラーログを短縮（LINE制限対応）
