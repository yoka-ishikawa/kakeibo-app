...
/**
 * Render環境の監視・デプロイ状況確認・自動通知サービス
 * Render REST APIを直接呼び出してデプロイ状況やログを監視
 */
@Service
public class RenderMonitoringService {
    ...
    public CompletableFuture<Void> startDeploymentMonitoring() {
        ...
        try {
            ...
            } else if ("build_failed".equals(latestDeployStatus) || "deploy_failed".equals(latestDeployStatus) || "update_failed".equals(latestDeployStatus)) {
    ...
    private String getRecentErrorLogs(String apiKey) {
        try {
            ...
            String url = RENDER_API_BASE_URL + "/logs" +
    ...
    private void sendDeploymentSuccessNotification() {
        try {
            ...
            // 既存のメソッドを使用
            notificationService.sendDeploySuccessNotification(
                "kakeibo-app", 
                "latest", 
                timestamp
            );
    ...
    private void sendDeploymentInProgressNotification(String status) {
        try {
            ...
            // 既存のメソッドを使用
            notificationService.sendDeployStartNotification(
                "kakeibo-app",
                "latest",
                timestamp
            );
    ...
    private void sendMonitoringErrorNotification(String errorMessage) {
        try {
            ...
            notificationService.sendConnectionErrorDetails(
                "Monitoring Error",
                errorMessage,
                "Render API monitoring failed at " + timestamp
            );
     ...
     * デプロイ失敗用Flexメッセージ作成
     */
    private String createDeploymentFailureFlexMessage(String status, String errorLogs, String timestamp) {
        try {
            // エラーログを短縮（LINE制限対応）
            String shortErrorLogs = errorLogs.length() > 100 ? 
                errorLogs.substring(0, 100) + "..." : errorLogs;
