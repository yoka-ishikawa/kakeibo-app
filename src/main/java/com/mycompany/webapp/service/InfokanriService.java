...
/** 家計簿データのビジネスロジックを処理するサービスクラス データアクセス層（Repository）とコントローラー層の中間層として機能 */
@Service
public class InfokanriService {
  ...
  /**
   * 指定されたIDの収支データを削除 存在しないIDの場合は何もしない（エラーを発生させない）
   *
   * @param id 削除対象のデータID
   */
  @Transactional
  ...
  /**
   * 収支データを更新 JPAのsaveメソッドはIDが設定されている場合、更新処理を実行
   *
  ...
  /**
   * レポート画面用のデータを取得 現在は全データを返す簡易実装。将来的には期間フィルタリング機能を実装予定
   *
