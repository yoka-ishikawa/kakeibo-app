...
/**
 * MCP LINE Bot連携サービス
 * 実際のLINE送信をMCPサーバー経由で実行
 */
@Service
public class MCPLineBotService {
    
    /** 
     * MCP LINE Bot経由でFlexメッセージをブロードキャスト送信
     * 環境変数でLINE_CHANNEL_ACCESS_TOKENが設定されている場合のみ実行
     */
    public boolean sendBroadcastFlexMessage(String altText, String flexContents) {
        ...
        if (channelAccessToken == null || channelAccessToken.isEmpty()) {
          ...
          try {
            ...
            System.out.println("Token: " + channelAccessToken.substring(0, Math.min(10, channelAccessToken.length())) + "...");
    ...
    private Map<String, Object> createFlexMessage(String altText, String flexContents) {
      ...
      /**
     * MCP LINE Bot経由でブロードキャスト送信を実行
     * 実際のMCP-LINE Bot APIを呼び出し
     */
    ...
    /**
     * MCPサーバー経由でLINE Bot APIを実行
     * 環境変数設定時のみ実際のAPI呼び出し、未設定時はモック
     */
